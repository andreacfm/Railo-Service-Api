<cfscript>component extends="mxunit.framework.TestCase" {
	...
	function testSimpleCreation(){
		var mailService = getObject('mail');
	...
	function testConstructorSendEmail(){
		...
		var mailService = getObject('mail',param);
		mailService.send();
	...
	function testSendAttachment(){
		var mailService = getObject('mail');
			...
			mailService.setBody("Hello Test2");
			mailService.setType("HTML");
			mailService.addParam(file=expandpath("/test_samples/query.csv"),type="text/plain",remove=false)
