...
component extends="mxunit.framework.TestCase" {
	...
	function setUp(){
		...
		variables.ftpConn = getObject('ftp',{server=ftp_server,username=ftp_username,password=ftp_pswd});
	...
	public void function testOpen(){
		var ftp = getObject('ftp',{server=ftp_server,username=ftp_username,password=ftp_pswd});
		var conn = ftp.open();
	...
	public void function testPutFile(){
		...
		variables.ftpConn.setLocalFile(expandPath('/test_samples/ftp1'));
