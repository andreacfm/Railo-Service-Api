...
component extends="mxunit.framework.TestCase" {
	...
	function testBaseFunctions(){
		...
		var urlname = "url"; 
		var httpItem = getObject('http',{url:urlname});
	...
	function testNamedQuery(){
		var urlname = "http://localhost:8888/test_samples/query.csv";
		var httpItem = getObject('http',{url:urlname, name:"colItems"}); 
		var results = httpItem.send();
	...
	function testSimplePost(){
		var urlname = "http://localhost:8888/test_samples/posttest.cfm"; 
		var httpItem = getObject('http',{url:urlname, method:"POST"}); 
		var results = httpItem.send();
